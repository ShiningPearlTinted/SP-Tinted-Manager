...
function getBridge(){ return document.getElementById('bridge'); }
...
function finishBridgeRequest(result){
  ...
  if(typeof request.success==='function') request.success(result);
...
function failBridgeRequest(message){
  ...
  if(typeof request.failure==='function') request.failure(message);
  setTimeout(flushBridgeQueue,0);
}
...
document.getElementById('loginForm').addEventListener('submit',function(event){
  ...
  callWebApp('login',{username:username,password:password},
    handleLoginResult,
    function(message){
      button.disabled=false;
      button.textContent='Sign In';
      setLoginError(message||'Unable to connect to SP Tinted Manager Web App.');
    }
  );
});
...
function handleLoginResult(result){
  ...
  if(!result||result.success!==true){
    button.disabled=false;
    button.textContent='Sign In';
    setLoginError(result&&result.message?result.message:'Invalid username or password.');
    return;
  }
...
function loadDashboard(){
  ...
  callWebApp('dashboard',{},handleDashboardResult,function(message){
    if(loading) loading.classList.add('hidden');
    console.error(message||'Dashboard request failed.');
  });
}
...
function handleDashboardResult(data){
  ...
  if(!data||data.success===false){
    console.error(data&&data.message?data.message:'Dashboard request failed.');
    return;
  }
...
function renderDashboard(data){
  ...
  document.getElementById('todayRegistration').textContent=data.todayRegistration??0;
  document.getElementById('totalVehicles').textContent=data.totalVehicles??0;
  document.getElementById('monthlyRegistration').textContent=data.monthlyRegistration??0;

  const tbody=document.getElementById('recent');
  const rows=data.recentCustomers||[];

  if(!rows.length){
    tbody.innerHTML='<tr><td colspan="5">No customer records found.</td></tr>';
    return;
  }
